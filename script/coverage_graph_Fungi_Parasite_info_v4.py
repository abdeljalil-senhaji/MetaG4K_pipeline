...
def main():
    ...
    inputFileV1 = extractInfoInputPath.split('/')[-1]
    ...
    with open(extractInfoInputPath) as extractInfoFile:
        ...
        for ligne in extractInfoFile:
            ...
            if species != current_species:
                if current_species is not None and len(coverList) > minReads:
                    ...
                    try:
                        ...
                        genus_id = next((k for k in ncbi.get_rank(lineage).values() if k == 'genus'), None)
                        genus = ncbi.get_taxid_translator([lineage[-2]]).get(lineage[-2], "sp.") if genus_id else "sp."
                    except:
                        genus = "sp."

                    dictListGenus[current_species] = genus
                    ...
                    plt.ylim(bottom=0)
                    plt.ylabel('Depth')
                    plt.xlabel(f"{dictListSpecies.get(current_species, 'Unknown')} (genus: {genus})", fontsize=10)
                    plt.tight_layout()
        ...
        # Dernière espèce après fin du fichier
        if current_species is not None and len(coverList) > minReads:
            coveragePercent = round(((len(coverList) - coverList.count(0)) / len(coverList) * 100), 5)
            dictListCoverage[current_species] = str(coveragePercent)
            dictListSizeGenome[current_species] = str(sizeSubject)
    ...
    with open(contigFilePath) as contigFile:
        contigFile.readline()  # skip header
    ...
    with open(outputPath, 'w') as count:
        for line in lineToCopy:
            ...
            count.write(f"{line},{dictListCoverage.get(taxID, 'N/A')},{dictListSizeGenome.get(taxID, 'N/A')},{genus},{numContigs},{uniqueCount}\n")
