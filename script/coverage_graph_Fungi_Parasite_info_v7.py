...
def main():
    ...
    print("[INFO] Connexion à la base NCBI...")
    ...
    with open(getNameInputPath) as getNameFile:
        for line in getNameFile:
            ...
            if len(split) >= 3 and split[1] != "UNKNOWN":
                taxID = split[0]
                speciesName = split[1]
                countReads = split[2]
                try:
                    name = ncbi.get_taxname_from_taxid(taxID)
    ...
    # Traiter extractInfo ligne par ligne
            ...
            if species != current_species:
                if current_species and len(coverList) > minReads:
                    ...
                    try:
                        ...
                        genus_taxid = next((k for k in lineage if ranks[k] == 'genus'), None)
                        if genus_taxid:
                            genus_name = ncbi.get_taxname_from_taxid(genus_taxid)
                        else:
                ...
                # Réinitialiser pour nouvelle espèce
